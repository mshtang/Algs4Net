...
public sealed partial class TextInput
{
    ...
    private void TryNextToken()
    {
        ...
        if (buffer != null)
        {
            var m = Regex.Match(buffer, @"\s*\S+\s*", RegexOptions.Compiled);
            //buffer = buffer.Substring(m.Value.Length);
            nextToken = m.Value.Trim();
        }
    }
    ...
    public bool HasNextInt()
    {
        ...
        if (buffer == null)
            return false;
        return int.TryParse(nextToken, out var dummy);
    }
    ...
    public bool ReadBool()
    {
        ...
        if (nextToken.Equals("true", StringComparison.InvariantCultureIgnoreCase))
            ...
            throw new FormatException($"error reading bool: {buffer}");
    }
    ...
    public string ReadLine()
    {
        ...
        if (!newBuffer)
        {
            var newLine = input.ReadLine();
            buffer = newLine is null ? "" : newLine;
        }
        s = buffer;
        buffer = "";
        return s;
    ...
    {
        return ReadLine() + input.ReadToEnd();
    }
